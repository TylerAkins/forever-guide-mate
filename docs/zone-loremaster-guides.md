...
- An item that starts an optional quest does not get a required step. Say to use the item on a step the player is already doing. Gate the turn-in, and every later step in that chain, with `quest` state `activeOrCompleted`. A missing item stays out of the percentage. Gating only the turn-in would open the follow-up, because a step the character cannot take yet counts as finished for the next step. Chen's Empty Keg is a barrel on the ground in the Barrens. Lakota'mani, Owatanka, Washte Pawne, and The Harvester are drops. Those are the examples.
